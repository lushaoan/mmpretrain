...
def parse_args():
    ...
    parser.add_argument(
        "--show-interval", "-i", type=float, default=2, help="the interval of show (s)"
    )
    parser.add_argument(
        "--mode",
        "-m",
...
def make_grid(imgs, names):
    ...
    for i, img in enumerate(imgs):
        if isinstance(img, list):
            for j in range(len(img)):
                ...
                subplot.set_title(
                    f"{name}\n{img_shapes[i][j]}", fontsize=15, family="monospace"
                )
        else:
    ...
    points = figure.get_tightbbox(figure.canvas.get_renderer()).get_points() + [
        [0, 0],
        [0, 0.5],
    ]
        ...
        for t in self.transforms:
            ...
            if "img" in data:
                ...
                if "mask" in data:
                    ...
                    img = [tmp_img] + img[1:] if isinstance(img, list) else tmp_img
                self.intermediate_imgs.append(
                    {"name": t.__class__.__name__, "img": img}
                )
        return data
...
def main():
    ...
    # init inspection
    intermediate_imgs = []
    dataset.pipeline = InspectCompose(
        dataset.pipeline.transforms, intermediate_imgs, visualizer
    )
    ...
    for i, item in zip(range(display_number), dataset):
        ...
        if args.mode == "original":
            ...
            image = make_grid([ori_image, trans_image], ["original", "transformed"])
        else:
            image = make_grid(
        ...
        out_file = (
            osp.join(args.output_dir, filename) if args.output_dir is not None else None
        )
